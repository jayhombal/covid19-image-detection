...
######### Functions used in Training Deep Learning Models #############
...
def get_base_model_with_new_toplayer(base_model,
    ...
    head_model = keras.layers.Dense(
        num_classes, activation=activation_func)(head_model)
    model = keras.Model(inputs=base_model.input, outputs=head_model)
    model = compile_classifier(
        model, learning_rate, optimizer='Adam', activation_type=activation_func)
    return model
...
def fine_tune_model(model, learning_rate=0.00001, optimizer='Adam',  fine_tune_at_layer: int = 178, activation_func: str = 'softmax'):
    ...
    compile_classifier(model, learning_rate=learning_rate,
                       optimizer=optimizer, activation_type=activation_func)
    return model
