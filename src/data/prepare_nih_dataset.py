...
class PrepareNIHData:
    ...
    def process_data(self, stable=True):
        ...
        all_image_paths = {os.path.basename(x): x for x in glob(
            os.path.join(image_path, 'images*', '*', '*.png'))}
        ...
        self.nih_xrays_df['path'] = self.nih_xrays_df['image_name'].map(
            all_image_paths.get)
        ...
        self.nih_xrays_df['finding_label'] = self.nih_xrays_df['finding_label'].map(
            lambda x: x.replace('No Finding', 'NoFinding'))
        ...
        all_labels = np.unique(list(
            chain(*self.nih_xrays_df['finding_label'].map(lambda x: x.split('|')).tolist())))
        # remove the empty label
        all_labels = [x for x in all_labels if len(x) > 0]
        self.logger.info('All Labels ({}): {}'.format(
            len(all_labels), all_labels))
        for label in all_labels:
            self.nih_xrays_df[label] = self.nih_xrays_df['finding_label'].map(
        ...
        group_shuffle_split = GroupShuffleSplit(
            n_splits=1, train_size=0.8, random_state=42)
    ...
    def write_data(self, processed_data_path):
        ...
        self.train_df.to_csv(
            'data/processed/prepared_train_data_entry_2017.csv', header=True, index=False)
        self.valid_df.to_csv(
            'data/processed/prepared_valid_data_entry_2017.csv', header=True, index=False)
